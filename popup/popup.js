...
function updateUI(settings, profiles, activeProfile, apiKeys, usage) {
    ...
    preview.innerHTML = `
      ...
      <div style="display: flex; flex-wrap: wrap; gap: 4px;">
        ${fields.slice(0, 3).map(f => `
          <span style="background: rgba(139, 92, 246, 0.1); padding: 2px 6px; border-radius: 4px;">
            ${f.label || f.type}
          </span>
        `).join('')}
  ...
  // Count API keys
  let keyCount = 0;
  Object.values(apiKeys).forEach(provider => {
    if (Array.isArray(provider)) keyCount += provider.length;
  });
  ...
  Object.values(usage).forEach(provider => {
    Object.values(provider).forEach(keyData => {
      if (keyData.lastUsed?.startsWith(todayStr)) {
        todayTokens += keyData.tokens || 0;
      }
    });
  });
  document.getElementById('todayUsage').textContent = formatTokens(todayTokens);
  
  // Update fields filled
